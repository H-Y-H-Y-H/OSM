...
def collect_sm_nondyna_data(step_num, step_each_epoch=6):
    S, A, NS = [], [], []
    obs = env.reset()
    count = 0
    for step in range(step_num//16+1):
        for sub_step in range(16):
            a = sin_move(sub_step, inital_para)
            a = np.random.normal(a,scale=0.5)
            obs_, r, done, _ = env.non_dyna_step(a)
            A.append(a)
            S.append(obs)
            NS.append(obs_)
            obs = np.copy(obs_)
...
class SAS_data(Dataset):
    def __init__(self, mode, num_data):
        ...
        self.all_S = np.loadtxt(log_path + 'S.csv')[:num_data]
        self.all_A = np.loadtxt(log_path + 'A.csv')[:num_data]
        self.all_NS = np.loadtxt(log_path + 'NS.csv')[:num_data]
...
def test_sm(sm_model, env, log_path, Action_array = None, TASK='f', eval_epoch_num=5, step_num=6, num_traj=256):
    sm_model.eval()
    log_action = []
    log_gt = []
    log_pred = []
    ...
    for epoch in range(eval_epoch_num):
        ...
        for step in range(step_num):
            ...
            if Action_array == None:
                A_array_numpy = np.random.uniform(-.5, .5, size=(num_traj, 16))
    ...
    np.savetxt(log_path + '/log_rewards.csv', np.asarray(log_results))
...
def test_non_dyna_sm(sm_model, env, log_path, Action_array = None, TASK='f', eval_epoch_num=5, step_num=6, num_traj=256):
    sm_model.eval()
    log_action = []
    log_gt = []
    log_pred = []
    ...
    for epoch in range(eval_epoch_num):
        ...
        for step in range(step_num):
            # scalse_noise = np.asarray([para_space] * num_traj).reshape((num_traj, 16))
            for sub_step in range(16):
                action = sin_move(sub_step, inital_para)
                action = np.random.normal(action, scale=0.5)
                A_array_numpy = np.asarray([action] * num_traj).reshape((num_traj, -1))
                ...
                # Only choose the action that has the largest reward.
                obs, r, done, _ = env.non_dyna_step(choose_a)
...
if __name__ == '__main__':
    ...
    if mode != 5:
        env = OSM_Env(dof, inital_para, para_space, urdf_path="../CAD2URDF/V000/urdf/V000.urdf",
    ...
    if mode == 1:
        sub_process = 2
        data_num = 600
        log_path = log_path + 'state_compare/%d_dyna_data/%d/' % (data_num, sub_process)
        os.makedirs(log_path, exist_ok=True)

        collect_sm_data(step_num=data_num)
        # collect_sm_nondyna_data(step_num=data_num)
        np.savetxt(log_path+"joint_state.csv", env.log_obs)

    # Train self-model
    sm_model = FastNN(18 + 16, 18) # activation_fun="Relu" dyna self-model
    # sm_model = FastNN(18 + 12, 18) # activation_fun="Relu" model-based
    if mode == 2:
        data_num = 600
        sub_process = 2 # sm_mode/data600/2
        log_path = log_path + 'state_compare/%d_dyna_data/%d/' % (data_num, sub_process)
    ...
    if mode == 3:
        data_num = 600
        sub_process = 2
        # log_path = log_path + '%d_data/%d/' % (data_num, sub_process)
        log_path = log_path + 'state_compare/%d_dyna_data/%d/' % (data_num, sub_process)
        # log_path = log_path + 'state_compare/%d_non_dyna_data/%d/' % (data_num, sub_process)

        sm_model.load_state_dict(torch.load(log_path + 'best_model.pt', map_location=torch.device(device)))
        sm_model.to(device)
        env.save_pth = None

        test_sm(sm_model, env, log_path, TASK='f', eval_epoch_num=100)
        # test_non_dyna_sm(sm_model, env, log_path, TASK='f', eval_epoch_num=3)
    ...
    if mode == 4:
        ...
        # pred = np.loadtxt(log_path + '/%d_data/%d/test/pred.csv' % (data_num,sub_process))
        # gt = np.loadtxt(log_path + '/%d_data/%d/test/gt.csv' % (data_num,sub_process))
        eval_data_num = 100
        pred = np.loadtxt('mpc_data/dof1200/state_compare/600_non_dyna_data/%d/test/pred.csv'%sub_process)[:eval_data_num]
        gt = np.loadtxt('mpc_data/dof1200/state_compare/600_non_dyna_data/%d/test/gt.csv'%sub_process)[:eval_data_num]
